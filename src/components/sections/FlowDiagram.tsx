  ...
  return (
    <section ref={containerRef} className="flow-section relative w-full h-screen bg-[var(--dark-bg)] overflow-hidden flex items-center z-10">
      ...
      <div ref={rowRef} className="flow-row relative z-10 flex items-center gap-10 md:gap-20 px-10 md:px-20 min-w-max">
        {/* Nodes */}
        {['Inverted Pyramid', 'Conversational Intent', 'Brand Entity', 'Schema Markup', 'Local SEO', 'Content Calendar', 'Dominance'].map((step, i) => (
      ...
      <div className="absolute bottom-10 left-10 md:bottom-20 md:left-20 max-w-[280px] md:max-w-md pointer-events-none z-20">
        <h3 className="font-sans font-light text-2xl md:text-3xl text-white mb-2 md:mb-4">AEO Strategy</h3>
        <p className="font-sans font-light text-sm md:text-base text-white/60">Structuring content to be the "source of truth" for AI engines.</p>
      </div>
    </section>
  );
}
